...
$X_{(2)},...,X_{(N)}$. The set of random variables $X_{(2)},...,X_{(N)}$ can be larger than or smaller than $x$ and still provide $X_{(1)} \leq x$. Note the cdf of the first-order statistic can also be expressed in the following form:
\begin{equation}
F_{X_{(1)}}(x) = 1 - P(\{X_{(1)} > x\})
\end{equation}
If $X_{(1)} > x$, $X_{(2)},...,X_{(N)}$ must be greater than $x$. Therefore, if  $X_{(1)} > x$, each of the random variables $X_1,...,X_N$ must be greater than $x$.
\begin{equation}
